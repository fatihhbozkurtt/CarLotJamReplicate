    ...
    public void GenerateGrid()
    {
        ...
        for (int x = 0; x < desiredRowCount; x++)
        {
            for (int y = 0; y < desiredColumnCount; y++)
            {
                ...
                cloneCellGO.GetComponent<CellController>().Initialize(x, y, cloneCellGO.transform.localPosition -= new Vector3(((desiredRowCount - 1) / 2f) * CellXLength, 0, -((desiredColumnCount - 1) / 2f) * CellYLength));
